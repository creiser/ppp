...
void init_qdct(void);
void qdct_block (const int8_t block[64], int8_t *output);
void iqdct_block(const int8_t *input   , int16_t block[64]);

int max_encoded_length(int n_values);
int compress_block  (const int8_t block[64], uint8_t *output);
int uncompress_block(const uint8_t *input, int8_t block[64], bool *dec_error);
int uncompress_data (const uint8_t *input, int max_input_len, int n_values,
                     int8_t *block, bool *dec_error);

int qdct_compress_block   (const int8_t block[64], uint8_t *output);
