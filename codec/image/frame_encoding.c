 ...
 * from uint8_t to int8_t (i.e., 128 is subtracted from each
 * pixel value).
 */
static void uint8_image_get_block(const uint8_t *image, int columns,
                           int block_nr, int8_t block[64]) {
    int yy, xx;
    block_to_row_col(block_nr, 0, columns, &yy, &xx);
    for (int y=0; y<8; y++)
        for (int x=0; x<8; x++)
            block[8*y+x] = (int8_t)((int)image[(yy+y)*columns + xx+x] - 128);
}
...
void init_qdct(void) {
    ...
    const float pi_16 = 0.19634955f;      /* M_PI/16.0f */
 ...
 * a little trick (remember that (A*B)^tr = B^tr * A^tr holds):
 ...
 */
void qdct_block(const int8_t *f, int8_t *ff) {
    float B[64], C[64];
 ...
 * return the 64 result values in 'f'. 'f' is
 * declared as an int16_t array (instead of int8_t)
 * because after the IDCT, values may be outside
 * of the int8_t range.
 *
 * The DCT is computed by the matrix product
 ...
 */
void iqdct_block(const int8_t *ff, int16_t *f) {
    float B[64], C[64];
...
static inline uint8_t get_nibble(const uint8_t *codes, int n,
                                 int max_nibbles, bool *dec_error) {
    uint8_t v;
    if (n >= max_nibbles) {
        *dec_error = true;
        return 0;
...
int uncompress_data(const uint8_t *codes, int max_input_length,
                    int n_values, int8_t *output, bool *dec_error) {
    const int max_nibbles = 2 * max_input_length;
    
    *dec_error = false;

#define nibble(n) (get_nibble(codes, (n), max_nibbles, dec_error))
    int i = 0, pos = 0;
    while (i < n_values) {
        ...
        if (c == 0) {
            ...
            output[i++] = - (int8_t)(nibble(pos++) + 3);
        else { 
            int v;
            v = nibble(pos++) << 4;
            v |= nibble(pos++);
            output[i++] = (c == 0xE) ? v+19 : -(v + 19);
        }
    }
 ...
 */
int uncompress_block(const uint8_t *codes, int8_t *output, bool *dec_error) {
 ...
 *   0xE:          next value is  ((n2<<4) + n3 + 19)
 *   0xF:          next value is -((n2<<4) + n3 + 19)
 *
 * Example:
 * The encoded sequence 0xC 0xB 0x2 0xA 0xF 0x2 0x1 0x0
 * is decoded as:
 *   0xC 0xB:     (0xB + 3) = 14
 *   0x2:         two zeros, i.e., 0 0
 *   0xA:         -1
 *   0xF 0x2 0x1: -((0x2<<4) + 0x1 + 19) = -(0x21 + 19) = -52
 ...
 */
int compress_data(const int8_t *input, int n_values, uint8_t *codes) {
 ...
 */
int compress_block(const int8_t *input, uint8_t *codes) {
 ...
 */
int qdct_compress_block(const int8_t *block, uint8_t *output) {
    int8_t dctres[64];
