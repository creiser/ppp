...
int decode_block(const uint8_t *data, int max_input_len,
    ...
    case PPP_IMGFMT_UNCOMPRESSED_DCT:
        ...
        iqdct_block((const int8_t *)data, decoded);
        off = 0;
        for (int y=0; y<8; y++)
            for (int x=0; x<8; x++)
    ...
    case PPP_IMGFMT_COMPRESSED_DCT:
        len = uncompress_data(data, max_input_len, 64, decompr, &dec_error);
        if (dec_error)
            fprintf(stderr, "Warning: block uncompression error for block at "
                    "top=%d, left=%d\n", yy, xx);
        iqdct_block(decompr, decoded);
        off = 0;
        ...
        if (is_intra(motion)) {
            ...
            for (int y=0; y<8; y++) {
                for (int x=0; x<8; x++) {
                    ...
                    int8_t v = (int8_t)old_image[pixel] - 128;
